...
def fig4_dataset_summary_table(stats_df, save_path):
    ...
    data_rows = [
        ...
        ('Audio Samples', int(hc_df['n_samples'].sum()), int(pd_df['n_samples'].sum()), int(stats_df['n_samples'].sum())),
        ('Samples/Subject ($\\mu$)', f"{hc_df['n_samples'].mean():.1f}",
         f"{pd_df['n_samples'].mean():.1f}", f"{stats_df['n_samples'].mean():.1f}"),
        ('Samples/Subject ($\\sigma$)', f"{hc_df['n_samples'].std():.1f}",
...
'''
    ...
    for i, (metric, hc_val, pd_val, total_val) in enumerate(data_rows):
        latex_code += f"{metric} & {hc_val} & {pd_val} & \\textbf{{{total_val}}} \\\\\n"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            if pdf_file.exists():
                ...
                # also create png version if requested
                if str(save_path).endswith('.pdf'):
                    png_path = str(save_path).replace('.pdf', '.png')
                    try:
                        subprocess.run(
                            ['convert', '-density', '300', str(save_path), '-quality', '100', png_path],
                            capture_output=True, timeout=30, check=False
                        )
                    except:
                        pass  # imagemagick not available, skip png
